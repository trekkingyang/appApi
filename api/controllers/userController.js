  ...
  User.findOne({
    ...
    if (!user) {
      ...
      if (!user.comparePassword(req.body.password)) {
        res.status(200).json({code:1,  message: 'Authentication failed. Wrong password.' });
      } else {
        return res.json({code:0,data: jwt.sign({ email: user.email, name: user.name, _id: user._id}, 'RESTFULAPIs')});
      }
    }
  });
}
